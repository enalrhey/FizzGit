...
function of_ajax_callback() 
{
	...
	elseif($save_type == 'restore_options')
	{
		...
		$tmg_data = get_option(BACKUPS);
		
		update_option(OPTIONS, $tmg_data);
	...
	elseif($save_type == 'import_options'){
		...
		$tmg_data = $_POST['data'];
		$tmg_data = unserialize(base64_decode($tmg_data)); //100% safe - ignore theme check nag
		update_option(OPTIONS, $tmg_data);
	...
	{
		wp_parse_str(stripslashes($_POST['data']), $tmg_data);
		unset($tmg_data['security']);
		unset($tmg_data['of_save']);
		update_option(OPTIONS, $tmg_data);
