		...
		if( isset( $zilla_shortcodes ) && is_array( $zilla_shortcodes ) )
		{
			...
			foreach( $this->params as $pkey => $param )
			{
				...
				switch( $param['type'] )
				{
					case 'headline' :
						$output  = $row_start;
						$output .= '<div style="font-weight:bold">'.$param["std"].'</div>' . "\n";
						$output .= $row_end;
					...
					case 'checkbox' :
						...
						$output .= '<input type="checkbox" value="on" class="zilla-input" name="' . $pkey . '" id="' . $pkey . '" ' . ( $param['std'] ? 'checked' : '' ) . ' />' . "\n";
			...
			if( isset( $zilla_shortcodes[$this->popup]['child_shortcode'] ) )
			{
				...
				foreach( $this->cparams as $cpkey => $cparam )
				{
					...
					$crow_start .= '<label>' . $cparam['label'] . ' ' .'<span class="child-clone-row-desc" style="display:inline;margin-left:10px;">' . $cparam['desc'] . '</span></label>'. "\n";
					$crow_start .= '</div>' . "\n";
					...
					switch( $cparam['type'] )
					{
						...
						case 'textarea' :
							...
							// prepare
							$coutput  = $crow_start;
							$coutput .= '<textarea rows="8" cols="30" name="' . $cpkey . '" id="' . $cpkey . '" class="zilla-form-textarea zilla-cinput">' . $cparam['std'] . '</textarea>' . "\n";
							$coutput .= $crow_end;
